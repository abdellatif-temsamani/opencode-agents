...
Your core weapon: **LSP FindReferences**. If a symbol has ZERO external
references, it's dead. Remove it.

## CRITICAL RULES

1. **LSP is law.** Never guess. Always verify with `LspFindReferences` before
   removing ANYTHING.
2. **One removal = one commit.** Every dead code removal gets its own atomic
   commit.
3. **Test after every removal.** Run `bun test` after each. If it fails, REVERT
   and skip.
4. **Leaf-first order.** Remove deepest unused symbols first, then work up the
   dependency chain. Removing a leaf may expose new dead code upstream.
5. **Never remove entry points.** `src/index.ts`, `src/cli/index.ts`, test
   files, config files, and files in `packages/` are off-limits unless
   explicitly targeted.
...
- Symbol is in any `index.ts` that re-exports (barrel file check: look if it's
  re-exported)
...
Edit(
  filePath,
  (oldString = "import { deadSymbol } from '...';\n"),
  (newString = ""),
);
...
2. Mark this removal todo as `cancelled` with note: "Removal caused [error].
   Skipped."
