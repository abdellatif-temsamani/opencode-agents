...
2. **Run pre-commit validation**:
   - Execute lint and report any issues on the project
   - Execute build and ensure it succeeds depending on the project
   - If either fails, ask user if they want to proceed anyway or fix issues
     first
...
4. **Analyze the changes**:
   ...
   - Analyze the diff to determine the primary change type (feat, fix, docs,
     etc.)
...
- **Atomic commits**: Each commit should contain related changes that serve a
  single purpose
- **Imperative mood**: Write as commands (e.g., "add feature" not "added
  feature")
...
- **Error handling**: If validation fails, give user option to proceed or fix
  issues first
- **Auto-staging**: If no files are staged, then ignore it, do not auto-stage
  just inform the user to stage the files
- **File priority**: If files are already staged, only commit those specific
  files
- **Always ask to push the commit**: You need to ask for confirmation unless
  there is a big issue or error `git push`.
- **Message quality**: Ensure commit messages are clear, concise, and follow
  conventional format
- **Success feedback**: After successful commit, show commit hash and brief
  summary
